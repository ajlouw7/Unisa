...
This study will attempt to create a model that can predict life expectancy for a country based on various socio-economic conditions in the country over a 30-year period. This will be done by implementing the various algorithms discussed in Section \ref{moddelingTechniques} in the Python language. Then these programs will be applied to the dataset. Python is a common tool used in machine learning applications, with a wide variety of libraries that can be used. The algorithms will be newly implemented for this project but libraries like Numpy will be used to help with reading data from files and writing data to files.
...
As discussed in Section \ref{moddelingTechniques} of the Literature Review, this study will compare k-Nearest Neighbour and Clustered linear regression with Linear regression. Linear Regression will be used as a baseline or control group for comparison on the dataset. 
...
Next, the data was run against 1261, 81 and 16 evenly spaces centroids in the solution space. These results can be seen in Table \ref{CentroidResults1296}, Table \ref{CentroidResults81} and Table \ref{CentroidResults16} respectively.
...
Run & Average Mean Error & Average Standard Deviation of Error\\
\hline
1 & 5.630419163 & 4.424955208\\
2 & 6.790878808 & 4.555293421\\
3 & 7.591301766 & 5.912122631\\
4 & 5.263780375 & 4.628624643\\
5 & 5.55373874 & 4.571667724\\
6 & 5.730609187 & 4.142707807\\
7 & 5.358985033 & 4.250570669\\
8 & 21.47563673 & 5.76594903\\
9 & 11.44550107 & 4.647011741\\
10 & 6.705006127 & 4.299021286\\
\hline
Average: & 8.1545857 & 4.719792416\\
\hline
\end{tabular}
\caption{Clustering algorithm results with 16 evenly spaced centroids}\label{CentroidResults16}
\end{table}
...
Run & Average Mean Error & Average Standard Deviation of Error\\
\hline
1 & 7.493788364 & 3.729178852\\
2 & 6.080383158 & 3.677490002\\
3 & 7.528946402 & 3.756068204\\
4 & 7.436940708 & 3.901789169\\
5 & 6.100301336 & 3.934587462\\
6 & 5.11928854 & 3.768635701\\
7 & 6.012460382 & 3.591611634\\
8 & 5.625271298 & 3.773165245\\
9 & 6.938321779 & 3.811118222\\
\hline
Average: & 6.481744663 & 3.771516054\\
\hline
\end{tabular}
\caption{Clustering algorithm results with 81 evenly spaced centroids}\label{CentroidResults81}
\end{table}
...
The weighted version of the algorithm did improve on the results of the unweighted algorithm. The weitghted algorithm had an average Mean Error of 2.115 years. Which is an improvement of 0.0051 years. The standard deviation of both kNN-Algorithms also improved more than 3 fold over the standard deviation of the errors for the OLS-Regression algorithm.
...
Randomly trying to find the appropriate initial starting position for the centroids was not successful. Therefor evenly spaced initial centroids were chosen. If the starting points were evenly spaced over the problem domain, then outlying data points should have their own centroids, thus improving predictive performance.

The next question was how small should those intervals be. Larger intervals means fewer centroids and will improve the running performance of the algorithm and enable more runs over more folds, but will pair data points that are more different together. Which makes the predictive performance worse. Conversely, a smaller interval leads to more centroids. Which would allow only more similar data points to be grouped together and thus improve the predictive performance of the algorithm, but at the cost of running performance. 
...
If an interval of 0.1 is chosen in normalised space, then that would mean $11^4 = 14641$ centroids that have to be processed, because the solution space is 4 dimensional. Therefor, an interval of 0.2 was chosen. Which equates to 1296 centroids. Which is a much more reasonable number. This one run took about a week and was done for all 10 folds. The results (Table \ref{CentroidResults1296}) show an average error of 9.909. Which is worse than even the Linear regression that was done first. This can be explained by some of the spaces (created by the intervals) have fewer predictors than the dimensions of the solution space. In this case $\beta$ (see Section \ref{regression}) has no unique solutions. Thus, giving unexpected results. A good example is where in fold 2 the test point for Norway in 2014 had a predicted life expectancy of 1126.056 years while in reality the life expectancy was 82.1 years. 

Trying to randomly find the optimum number of centroids and the optimum initial placement of these centroids, did not yield better results than the kNN algorithm (See Table \ref{kNNResultsWeighted}). Having more runs could have yielded better results. But, given the limited computing power available, another approach was followed. 

The number of centroids were reduced while the number of runs were increased. Fewer centroids would alleviate the problem of not having enough data points per centroids and reducing the total run time of the algorithm. While increasing the number of runs would ensure that the results are not based on one particular way the data was grouped into folds. The number of centroids were chosen to be 81, which corresponds to an interval between centroids of 0.5 in the normalised space. This is the largest the interval could be before only peripheral points on the space is used. Then the interval would be 1.0. An interval between 0,5 an 1.0 can not be considered, because that would make the centroids bias to one side of the solution space.

The results for this run can be found in Table \ref{CentroidResults81}. Note only 9 runs were done compared to the standard of 10 runs per algorithm. This was due to time constraints. Comparing the results of an interval width of 0.5 (no of centroids = 81) to that of an interval width of 0.2 (no of centroids = 1296), reveals that the larger interval gave better results. The average error for having 81 clusters was 6.48 years while the average for 1296 clusters were 9.91 years. 

Next was a run of 16 centroids with a interval width of 1.0. This is where all the peripheral points are used including the centre point. This can be seen in Table \ref{CentroidResults16}. The performance was slightly worse than the 81 centroids at 8.15 average error in life expectancy. One potential reason for this is that there were more data points per centroid which meant that the linear approximation could be worse since some of the data points could be far from the calculated centroid.

From all the clustering runs with evenly placed initial centroids, the one with 81 centroids performed the best. This appears to be the sweet spot from the analysis that was done. Enough data points are at each centroid so that the linear regression can be computed correctly and enough centroids are available such that each data point could be close enough to a centroid so that the linear approximation is accurate.

All of these average errors were still larger than the average error determined by the weighted kNN-algorithm.

\section{Conclusions and limitations}



Multi-dimensional data is hard to visualise and thus hard to understand and study. The situation gets worse with each additional dimension. Combining Primary- , Secondary- and Tertiary School Enrollment, meant that the amount of dimensions studied reduced from 7 to 5. Which is unfortunately still difficult to analyse, mostly because it is impossible to visualise mentally. Techniques like a matrix scatter plot from Figure \ref{scatter} helped but still require interpretation.

The source code and raw results can be found on GitHub \citep{rawData}.

The greatest limitation for the analysis of the centroid algorithm was the computation time. Running through all the folds of a dataset could take more than a week, not including external factors like load shedding or technical issues on the chosen computer. The Python language is not always an ideal language to scientific computation when all the computation, especially long loops, are not being done in a more performant language like c or c++ \citep{Cai2005}. It would have been more efficient to implement the clustering algorithm in a more performant language and then calling those libraries from python. If that was the case then the algorithm could have been tested against more clusters. The threshold distance that the data points had to be from the centroids, for the algorithm to terminate, could also have been reduced. 
...
Even with the limitations, kNN was the clear winner. It has the ability to localise the datapoints that would make the greatest contribution to a correct prediction. While being able to ignore outliers that could affect prediction negatively.

Even with the limitaions, both k-Nearest Neighbour and Linear Clustering created models that were better at predicting Life Expectancy than the Linear Regression algorithm.  


\section{Recommendations}

Python is a great language to do machine learning in. Libraries like numpy make it easy to handle data sets. Unfortunately, heavy logic should not be implemented in python and should rather be implemented in a library, in a more performant langauge, which python can then utilise. 
