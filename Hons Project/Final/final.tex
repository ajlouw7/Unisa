...
The following procedure was followed in order to get 10 stratified folds. The dataset was then sorted according to decending life expectancy. The dataset was then split in half such that the life expectancy of each data point in the ``top'' data set is higher than the life expectancy of each data point in the ``bottom'' data set. Then 10 datasets of equal size was created from the ``top'' and ``bottom'' data sets, by taking 53 data points from each set. Thus each fold has 106 data points, with 53 high life expectancy points and 53 low life expectancy points.  

\subsection{Linear Regression Results}

As described in Section \ref{regression} the OLS algorihm was implemented in Python and run for each fold of the data. The results can be see in Table \ref{OLSResults}. The average of the mean error for each fold was 9.598848355. 

\begin{table}
\begin{tabular}{|c|c|c|}
\hline
Fold & Mean Error & Standard Deviation of Error\\
\hline
1  & 10.29874164 & 9.017297877 \\
2  & 9.081106784 & 6.890622426 \\
3  & 9.125215292 & 7.464715809 \\
4  & 8.356464123 & 6.178963524 \\
5  & 9.975440028 & 7.974842017 \\
6  & 8.703226052 & 6.236500583 \\
7  & 9.56450367  & 6.710214979 \\
8  & 10.73726516 & 8.829969189 \\
9  & 9.546407917 & 8.136121573 \\
10 & 10.60011289 & 8.510052534 \\
\hline
Average: &9.598848355 & 7.594930051 \\
\hline
\end{tabular}
...
\end{tabular}
\caption{Mean Error in Predicted Life Expectancy per Fold for the kNN Algortihm}\label{kNNResultsPerFold}
\end{figure}
...
\hline
Fold & k & Mean Error & Standard Deviation of Error\\
\hline
1 & 	32 & 	2.35979959	 & 2.94988107  \\
2 & 	16 & 	2.168031418	 & 1.806913134 \\
3 & 	31 & 	1.974412948	 & 2.721274262 \\
4 & 	39 & 	1.882756335	 & 2.408243797 \\
5 & 	25 & 	1.550020316	 & 1.271036442 \\
6 & 	4 & 	1.748667376	 & 1.891826963 \\
7 & 	64 & 	2.321097425	 & 3.279927484 \\
8 & 	41 & 	2.150680693	 & 2.185943479 \\ 
9 & 	4 & 	2.411306404	 & 2.073836258 \\
10 & 	48 & 	2.261508095	 & 3.010802076 \\
\hline
Average: & 30.4& 2.08282806 & 2.359968496 \\
\hline
\end{tabular}
\caption{kNN Results tabulating the value of k for which each fold had the smallest error}\label{kNNResults}
\end{table}
...
\begin{tabular}{|c|c|c|}
\hline
Fold & Mean Error k = 30 & Mean Error k = 31\\
\hline
1 & 2.368749226	&2.361272642\\
2 & 2.243774499	&2.254901804\\
3 & 1.990101446	&1.974412948\\
4 & 1.966454765	&1.951704915\\
5 & 1.586040998	&1.570644923\\
6 & 1.790011747	&1.801034007\\
7 & 2.34855691	&2.347208751\\
8 & 2.187162436	&2.174740754\\
9 & 2.506916164	&2.51679621\\
10 &2.34591783	&2.348668221\\
\hline
Average: & 2.133368602 & 2.130138517 \\
\hline
\end{tabular}
\caption{kNN Results for k = 30 and k = 31}\label{kNNResultsk=3031}
\end{table}

\subsection{Clustering Results}
At first, random centroids were selected for each run of the clustering algorithm on each fold. This was also done for various numbers of centroids. The runs ran for up to 15 centroids per fold. The amount of centroids that could be used was limited by the amount of time the algorithm took to run. It took approximately 12 hours to run through 2 to 15 centroids for each fold. 4 of these total runs were done.


\section{Discussion}

Since Linear regression is used often in the literature (Section \ref{regression}), the result for linear regression will be regarded as the baseline. When using 10 folds, the average of the mean error per fold was 9.599 with an average standard deviation per fold of 7.599. This means that on average the prediction that OLS regression made for the life expectancy of a country, based on its indicators, was off by 9.6 years. The large error is to be expected since the data is not linear, as can be seen in Figure \ref{scatter}. More specifically, the Preston Curve discussed in Section \ref{income} can be seen in Figure \ref{scatter} where Life Expectancy and GDP per Capita intersect. Thus Linear Regression when done over the whole dataset, gives such a bad result. 

The kNN-alogortihm fared much better. Table \ref{kNNResults} indicates which value of k performed the best for each fold. The average error when the optimum k is used for each fold is 2.083 years. But, if we consider only one value of k across all the folds, then the optimum value of k is 41 with an average mean error of 2.120 years. As we can see from Figure \ref{kNNResultsAllFolds}, the error was at its highest when k=1 and then rapidly drops until it reaches between k=20. Then it slowly drops til around k=40 and then slowly starts to rise. Unlike OLS Linear regression, the kNN-algorithm only takes datapoints into account that are local to the point in question. Therefor, not caring about the general structure of the whole dataset. One disadvantage of this algorithms was the need to run the data for so many different values of k. The max value of k was 106. This meant that for each fold the kNN-algorithm was run for 1 Nearest Neighbour up to all the Neighbours in the fold. Given that a fold only contained 106 data points. Another disadvantage was the algorithm taook longer to run every time the value of k was increased.
